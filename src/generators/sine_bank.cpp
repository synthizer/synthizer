 ...
 * */
static void sigmaApproximate(std::vector<SineWaveConfig> &waves) {
  ...
  double m = waves.back().freq_mul + 1;
  ...
  for (auto &w : waves) {
    double n = w.freq_mul;
    // Sinc function.  We know this can never be zero.
 ...
 * */
static void normalizeSeries(std::vector<SineWaveConfig> &waves) {
  double gsum = 0.0;
  for (auto &w : waves) {
    gsum += w.gain;
  }
...
static std::vector<SineWaveConfig> buildSquare(unsigned int partials) {
  unsigned int total_waves = partials + 1;
  for (unsigned int i = 0; i < total_waves; i++) {
  }
  return {};
}
