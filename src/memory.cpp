...
std::shared_ptr<CExposable> getExposableFromHandle(syz_Handle handle) {
	...
	if (out == nullptr || out->isPermanentlyDead()) {
		throw EInvalidHandle("This handle is already dead. Synthizer cannot catch all cases of invalid handles; change your program to use only valid handles or risk crashes.");
	}
	return out;
}
