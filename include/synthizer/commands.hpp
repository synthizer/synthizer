...
void initCallbackCommand(Command *cmd, ARGS ...args) {
 ...
 * weakened to weak_ptr equivalents.  They're repromoted to shared_ptr at execution time, and the command is optionally short-circuited
...
namespace referencing_cmd_details {
	...
	template<typename T>
	std::weak_ptr<T> weaken(std::shared_ptr<T> v) {
 ...
 * 
 * This is used for, e.g., adding generators to sources, where we can't proceed if either the source or generator no longer exists.
 * */
template<typename CB, typename ...ARGS>
void initReferencingCallbackCommand(Command *cmd, bool short_circuit, CB callback, ARGS ...args) {
	...
	referencing_cmd_details::initReferencingCallbackCommandAlreadyWeakened<std::remove_reference_t<CB>>(cmd, short_circuit, callback, referencing_cmd_details::weaken(args)...);
}
