 ...
 * 16 default constructor calls even though no items are used yet.  It's possible to change this, but for the sake of implementation simplicity we opt not to until performance becomes a problem.
 * */
...
class SmallVec {
	...
	auto end() {
		return this->storage.begin() + this->length;
	}
