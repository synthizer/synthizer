 ...
 * 
 * Finally, for advanced users, readFrame and writeFrame exist in the appropriate cases and will return pointers to the buffer itself.
 * */
...
class BlockDelayLine {
	...
	class Reader {
		...
		FLATTENED void runLoop(unsigned int done, unsigned int samples, FUNCTOR &&f) {
	...
	template<typename F>
	FLATTENED void runReadLoop(unsigned int max_delay, F &&closure) {
	...
	FLATTENED void runReadLoopSplit(unsigned int max_delay, unsigned int first, F1 &&first_c, unsigned int second, F2 &&second_c) {
	...
	FLATTENED void runLoopPrivSplit(unsigned int max_delay, unsigned first, F1& first_c, unsigned int second, F2 &second_c) {
		...
		if (needs_modulus) {
			...
			if(second) r.runLoop(first, second, second_c);
		} else {
