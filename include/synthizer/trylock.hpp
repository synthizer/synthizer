 ...
 * 
 * This also protects data in a Rust-ish fashion: you provide a pair of callbacks, and the callbacks get called instead of being able to try_lock directly.
 * */
...
class TryLock {
	...
	TryLock(ARGS&& ...args): data(std::forward(args)...) {}
	...
	bool try_lock() {
		...
		if (this->locked.load(std::memory_order_relaxed) == 1 ||
			this->locked.compare_exchange_strong(expected, 0, std::memory_order_acquire, std::memory_order_relaxed) == false) {
			return false;
		}
		return true;
	}
	...
	void unlock() {
		this->locked.store(0, std::memory_order_release);
	}
