...
inline static drmp3_bool32 seek_cb(void *user_data, int offset, drmp3_seek_origin origin) {
  ByteStream *stream = (ByteStream *)user_data;
...
inline Mp3Decoder::Mp3Decoder(std::shared_ptr<LookaheadByteStream> stream) {
  stream->resetFinal();
  this->stream = stream;

  if (drmp3_init(&this->mp3, read_cb, this->stream->supportsSeek() ? seek_cb : NULL, this->stream.get(), NULL) ==
      DRMP3_FALSE) {
...
inline Mp3Decoder::~Mp3Decoder() { drmp3_uninit(&this->mp3); }

inline unsigned long long Mp3Decoder::writeSamplesInterleaved(unsigned long long num, float *samples,
...
inline void Mp3Decoder::seekPcm(unsigned long long pos) {
...
inline bool Mp3Decoder::supportsSeek() { return this->stream->supportsSeek(); }

inline bool Mp3Decoder::supportsSampleAccurateSeek() { return this->supportsSeek() && true; }
