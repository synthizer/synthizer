...
class MpscRing {
	...
	bool write(CB &&callback);

	/* Returns false if the ring is full. */
	bool enqueue(const T&& value);

	template<typename CB>
	void processAll(CB &&callback);
	private:
...
template<typename T, std::size_t capacity>
template<typename CB>
bool MpscRing<T, capacity>::write(CB &&callback) {
	/*
	...
	while (1) {
		...
		if (this->ring[index].state.compare_exchange_strong(state, MPSC_RING_E_ALLOCATED_FOR_WRITE, std::memory_order_acquire)) {
...
template<typename T, std::size_t capacity>
bool MpscRing<T, capacity>::enqueue(const T&& value) {
	return this->write([&](T& val) {
		val = value;
	});
}
