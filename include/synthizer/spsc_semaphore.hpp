...
/*
 * This is a single-producer single-consumer semaphore optimized for low concurrency and frequent construction/destruction.
 * 
...
class SPSCSemaphore {
	...
	~SPSCSemaphore() {
		SPSCSemaphore::sema_pool.push_back(this->backing_sema);
	}

	void signal() {
		this->backing_sema->signal();
	}

	void wait() {
		this->backing_sema->wait();
	}
