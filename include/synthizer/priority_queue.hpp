...
class PriorityQueue {
	...
	/**
	 * Pop an element from the heap, returning whether an element was popped and calling the provided closure with the element.
	 * */
	 ...
	 * */
	template<typename CL>
	void popUntilPriority(PRIO priority, CL &&closure) {
		while (true) {
			if (this->elements.empty()) {
				return;
			}
	 ...
	 * */
	template<typename CL>
	void filterAllItems(CL &&closure) {
