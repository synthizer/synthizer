 ...
 * 
 * Events only actually fire if all of the strongly-referenced handles are still alive. This allows automatically dropping any pending
 * events that refer to deleted objects without having to deal with having individual per-object queues, which would have significant overhead.
 * */
class PendingEvent {
	public:
	/**
...
class EventBuilder {
	...
	private:
	/**
	 * Associate a handle with this event, return whether or not the object was valid (e.g. non-null, still visible from C)
	 * */
