 ...
 * 
 * It is permissible for the callbacks here to have side effects.  This interface isn't intended to be pure, only to get a particular coding idiom
 * out of the way of more important logic.  In particular the condition is called on every call to evaluate.
 * */
 ...
 * 
 * This probably isn't the class you want to use; see EdgeTrigger, below, which provides an abstraction that hides the heap allocations,.
 * */
class EdgeTriggerBase {
	...
	bool shouldTriggerUp() {
		return this->type != EdgeTriggerType::Down;
	}

	bool shouldTriggerDown() {
		return this->type != EdgeTriggerType::Up;
	}
